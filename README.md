...
<script>
    ...
    const defaultData = [
        ...
        { id: 5, tag: 'もくもく', loc: '由布岳・湯布院エリア', desc: '朝霧や由布岳周辺に広がる雲、そこで温泉の湯気がもくもくと立ち上る絶景。', lat: 33.2842, lng: 131.3917, img: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?w=600' },
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // 現在地へ自動ジャンプ
                mainMap.setView([userLat, userLng], 12);
                
                // 現在地にピンを立てる
                L.marker([userLat, userLng]).addTo(mainMap)
                    .bindPopup('<b>現在地（あなたは今ここにいます）</b>').openPopup();
            },
            (error) => {
    ...
    postForm.addEventListener('submit', (e) => {
        ...
        if (file) {
            resizeAndCompressImage(file, function(compressedImage) {
                ...
                try {
                    ...
                    alert("ストレージ容量の上限を超えました。不要な古い投稿を削除してください。");
                    locationData.pop();
                }
            });
        }
