...
<script>
    ...
    // ★ 本物の現在地（GPS）を取得する処理を追加
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                const userLat = position.coords.latitude;
                const userLng = position.coords.longitude;
                // 現在地が取得できたら、地図をそこへ移動させる（ズームレベル12）
                mainMap.setView([userLat, userLng], 12);
                
                // 現在地に青いマーカー（マイロケーション）を立てる
                L.marker([userLat, userLng]).addTo(mainMap)
                    .bindPopup('<b>あなたは今ここにいます</b>').openPopup();
            },
            (error) => {
                console.warn("位置情報の取得に失敗しました:", error.message);
                // 拒否された場合や電波エラー時はデフォルトの大分表示のまま進む
            },
            {
                enableHighAccuracy: true, // 高精度なGPSを要求
